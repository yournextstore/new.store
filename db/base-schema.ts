import type { OrderData } from "@/dao/order";
...
export const StoreEnvironmentEnum = pgEnum("store_environment", ["live", "test"]);
export const StoreTaxBehaviorEnum = pgEnum("store_tax_behavior", ["inclusive", "exclusive"]);
...
export const storesRelations = relations(stores, ({ one, many }) => ({
	...
	organization: one(organizations, { references: [organizations.id], fields: [stores.organizationId] }),
}));
...
export const pageSectionData = pgTable(
	...
	(t) => [
		...
		unique("psd__stripe_product_id__page_section_pointer__unique").on(t.productId, t.pageSectionPointer),
	],
);
...
export const questions = pgTable("questions", {
	...
	answers: jsonb("answers").notNull().default("[]").$type<JSONContent>(),
...
export const products = pgTable(
	...
	(t) => [
		...
		index("products__summary__gin").using("gin", sql`${t.summary} gin_trgm_ops`),
	],
);
...
export const variantTypes = pgTable(
	...
	(t) => [unique("variants__product_id__label__unique").on(t.productId, t.label)],
);
...
export const variantValues = pgTable(
	...
	(t) => [unique("variant_values__variant_type_id__value__unique").on(t.variantTypeId, t.value)],
);
...
export const carts = pgTable("carts", {
	...
	addonData: jsonb("addon_data").$type<Record<string, unknown> | undefined | null>().notNull(),
...
export type OrderStatusEnum = typeof OrderStatusEnum extends PgEnum<infer R> ? R : never;
...
export const orders = pgTable(
	"orders",
	{
		...
		orderData: jsonb("order_data").$type<OrderData>().notNull(),
...
export const AddressTypeEnum = pgEnum("address_type", ["shipping", "billing", "merchant"]);
...
export const lineItems = pgTable(
	...
	(t) => [unique("line_items__cart_id__variant_id").on(t.cartId, t.productVariantId)],
);
...
export const productsTaxRatesRelations = relations(productsTaxRates, ({ one }) => ({
	product: one(products, {
		references: [products.id],
...
export const shippingsTaxRatesRelations = relations(shippingsTaxRates, ({ one }) => ({
	shipping: one(shippings, {
		references: [shippings.id],
